...
\subsection{Classification of bilinear forms over a local algebra}%<<<2
...
% further assume that $D$~is diagonal (they show that the general case is
...
with~$H$.
However, $Z = D · Y · \transpose{Y} · D^{-1} = \mat{d & 0 \\ 0 & d} = W^2$
where $W = \mat{0 & d/w \\ w & 0}$ for any~$w ≠ 0$.
We now easily check that
$X = Y · W^{-1}$~is not a solution of the IP1S problem.

In the real case, the “polar decomposition” method computedsthe
