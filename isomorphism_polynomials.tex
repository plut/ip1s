...
of~$(b_{λ})$. An element~$x = x_0 + λ x_1 + … + λ^h x_h$ is a
relation iff
...
b_{∞} x_{h} = 0.
\end{equation}
A \emph{minimal relation} for~$(b_{λ})$ is one with minimal degree~$h$;
...
basis~$(w_1,…,w_r)$ such that, if $h_i$~is the degree of the
relation~$w_i$, then $(w_i,…,w_r)$~generate no relation of degree~$<
h_i$. The degree~$h_i$ are called the \emph{minimal indices} of the
pencil~$(b_{λ})$.
%>>>
...
\end{split}
\end{equation}
This means that~$(e'_0,…,e'_{h-1})$ is a relation of degree~$≤ h-1$
...
in this case we see that~$(e_1,…,e_h)$ is a relation of degree~$≤ h-1$
for~$(b_{λ})$.
\end{proof}
%>>>
...
\begin{proof}
Let~$(e_0, …, e_d)$ be a minimal relation. By
Prop.~\ref{prop:minimal-indep}, there exist vectors~$f_1,…,f_d$ such
that~$b_0(e_i, f_j) = 1$ when~$i = j$ and~$0$ otherwise, and the
family~$(e_0,…, e_d, f_1, …, f_d)$ is free. In any basis completing this
...
of~$\transpose{\ro B}$ defines a relation of degree~$≤ h-1$
on~$(b_{λ})$~\cite[XII(10)]{Gantmacher2}.
