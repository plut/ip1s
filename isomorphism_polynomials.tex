...
section~\ref{S:bil-sing}. This uses the Kronecker classification of
...
the minimal polynomial of~$c$ is primary, \emph{i.e.} of the form~$f^ℓ$
where $f$~is irreducible. In particular, since $k$~is a finite field,
...
on~$V$ such that~$b_{∞} = \Tr_{K/k} ∘ τ_ℓ ∘ b_R$.
...
We give a classification of $τ$-alternating forms, up to congruence. This
is not directly useful for the IP1S problem. XXX
...
\begin{proof}
Since the polar form of~$q$ is hyperbolic, there exist some
coefficients~$a_1, …, a_{2m} ∈ R$ such that $q$~is isomorphic to the form
$[a_1, …, a_{2m}]$ on~$R^{2m}$ defined by
\begin{equation}
q(x_1, x_2, x_3, x_4) \;=\;
  τ(∑_{i ≤ 2m} a_i σ(x_i) + ∑_{i ≤ m} γ(x_{i} x_{i+m})).
\end{equation}
To prove the proposition, it is enough to show that any form~$[a_1, a_2,
a_3, a_4]$ is isomorphic to a form~$[a, 0, b, 0]$ for some~$a, b ∈ R$.
...
(a_3, a_4) ← (a_3, a_4) σ(\transpose{M}^{-1}) \end{cases}\\
\mat{1&&u\\&1&&v\\ &&1&\\ &&&1} &
...
There exists a polynomial algorithm computing linear equivalence between
quadratic pencils in characteristic two.
\end{prop}

\begin{proof}
As shown by what precedes, this reduces to computing equivalence between
two $K$-quadratic forms computing with~$R$ on a free $R$-module of
bounded rank. Moreover, we may assume that both modules are hyperbolic.
...
with the same hyperbolic polar form~$b$, and with diagonal
coefficients~$a'_i$.

An isomorphism~$u: q → q'$ is given by coefficients~$u_{i,j}$ satisfying
the equations of Prop.~\ref{prop:quad-changevar}. Writing~$u = V(v) + π
V(w)$, and likewise for~$u'$ and~$u_{i,j}$, we have
\begin{equation}
σ (u) = v^2 + π w^2, \qquad
...
θ(a γ (u)) = v\, w\, θ(a π),
\end{equation}
so that the equations on the variables~$u_{i,j}$ are equivalent to the
following polynomial equations in the variables~$v_{i,j}$ and~$w_{i,j}$:
...
This shows that the IP1S problem is equivalent to a bounded number of
polynomial equations in a bounded number of variables in the ring~$R =
R_{ℓ} = K[π] /π^{ℓ}$
(more precisely, it is enough to determine~$v_{i,j}$ up to
precision~$\floor{ℓ/2}$ and~$w_{i,j}$ up to precision $\floor{(ℓ-1)/2}$).
Since $R$~is a discrete valuation ring, linear equations are solvable
in~$R$ in the sense of~\cite[4.1.5]{adams1994grobner};
...
However, in our case there are exactly 32~variables
