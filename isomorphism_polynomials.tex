...
centuries~\cite{lidl1997finite}. In the non-homogeneous case, the
...
Define matrices~$K'_0, K'_{∞}$ of size~$(d+1)×d$ and $K_{0}, K_{∞}$ of
size~$(2d+1)×(2d+1)$ by
...
where $B'_{λ}$ is a symmetric pencil with minimal index at least~$d$.
\end{prop}

Note in particular that the case~$d = 0$ corresponds to the matrix~$K_0$,
...
blocks~$(K_{d_1}, …, K_{d_r}, B')$, where $K_d = K_{d,0} + λ K_{d,∞}$~is
the square matrix of size~$2d+1$ defined by equation~\eqref{eq:def-K},
...
for integers~$d ≥ 0$, as defined in Prop.~\ref{prop:kronecker};
\item local blocks~$L_{p,d,u}$, defined as the~$d × d$-block matrix
...
First of all, we need to compute the Kronecker form of the two pencils $a_{\lambda}=\lambda a_{\infty}+a_0$
and $b_{\lambda}=\lambda b_{\infty}+b_0$. This part is essentially a gaussian elimination as it is explained in section 2. 
Now, we concentrate on the regular part of the pencils or assume that pencils are regular and 
can be written as $a_{\infty}(\lambda+m)$ and $b_{\infty}(\lambda+m)$ with $m={b_{\infty}}^{-1}b_0$ up to a change of variables on $a_{\infty}$ as it is explained in equation (3.4). Then, we compute the invariant factors of $m$ to find the block decomposition of $a_{\infty}$ and $b_{\infty}$ into a canonical form. Next, we compute the normal form of the two pencils using local block $L_{p,d,u}$ as described in section 3 using Proposition~3.26 and Theorem~3.30. All in all, every step has a 
complexity $O(n^3)$ since they require elementary operations on the matrices.

\begin{algorithmic}[1]
\Function{IP1SAlgo}{$a_{\lambda}$, $b_{\lambda}$ pencils}
  \State Compute Kronecker decomposition
  \State Write the regular parts as $a_{\infty}(\lambda+M)$ and $b_{\infty}(\lambda+M)$ using a change of variables
  \State Compute the invariant factors $p^d$ of the characteristic endomorphism $M$
  \State Compute the decomposition of regular pencils using the local block $L_{p,d,u}$ for each factor 
\EndFunction
\end{algorithmic}
...
the general case, several roadblocks remain: the Kronecker form for the
singular part of a bilinear pencil does not apply, as the proof of
Prop.~\ref{prop:kronecker1} above requires that~$2 ≠ 0$; quadratic forms
over a local algebra behave differently~\cite[§93]{omeara}; finally,
extending from bilinear to quadratic forms requires a study of the action
of a symplectic group on the diagonal coefficients, and this group
becomes quite impractical in the non-cyclic case.

On the other hand, studying the general problem with $m ≥ 3$~quadratic
equations departs from the classical results about pencils of quadratic
