...
forms} over~$V$, or a \emph{symmetric pencil} in short, is a
...
%>>>1
\section{The singular part of quadratic pencils}%<<<1
...
\subsection{The Kronecker decomposition of the singular part of a pencil} %<<<2
...
This last computation shows that the proof also holds in characteristic
...
blocks~$(K_{h_1}, …, K_{h_r}, B')$, where $K_h$~is
...
%>>>
%>>>1
\section{Equivalence of regular pencils in the IP1S sense}%<<<1
\label{S:IP1S-regular}

We give here an algorithm for solving the IP1S problem in the case of two
regular pencils. Assume that $\bm{b} = (b_{λ})$~is regular, which means
that its characteristic polynomial~$f(λ) = \det (b_0 + λ b_{∞})$ is not
zero. Then, for any~$λ$ such that~$f(λ) ≠ 0$, the bilinear form~$b_{λ}$
is regular.
...
implies that $b_0$~has an adjoint endomorphism~$m = -b_{∞}^{-1} b_0$ such
that~$b_0(x,y) = -b_{∞}(x, m y) = -b_{∞}(m x, y)$; in particular, all
elements of the algebra~$k[m]$ are self-adjoint with respect to~$b_{∞}$.
...
\end{split}
\end{equation}
Since~$y' = m y$ also verifies~$h(m)(y') = 0$, equation~\eqref{eq:bezout}
...
\subsection{Structure of local bilinear pencils}

Let~$b_{λ} = λ b_{∞} + b_0$ be a local pencil. We may use the
characteristic endomorphism~$m = - b_{∞}^{-1} b_0$ to write~$b_{λ}$ in
the form~$b_{λ} = b_{∞} (λ - m)$. Since $b_{λ}$~is local, we know that
the minimal polynomial of~$m$ is primary, \emph{i.e.} of the form~$p^d$
where $p$~is irreducible. In particular, since $k$~is a finite field,
this implies that $p$~is a separable polynomial.

We define the local algebra~$R_d = k[x] / p(x)^d$
and the extension field~$K = R_1 = k[x] / p(x)$. For each~$d$, $R_d$~is
isomorphic to~$K[π]/π^d$. Namely, a morphism~$K → R_d$ is given by a
root~$z$ of~$p$ in~$R_d$; the root~$x$ of~$p(x)^d$ is an approximate
solution, and by Hensel's lemma this may be lifted to an exact solution.


Let~$A → B$ be a morphism of (commutative) rings and let~$M$ be a
$B$-module. We say that a $A$-bilinear form~$b: M × M → A$ \emph{commutes
with~$B$} if, for any~$a ∈ A$ and~$x, y ∈ M$, we have~$b(x, ay) = b(ax,
y)$.

Using this definition we see that the local pencil~$b$ defines a bilinear
form~$b_{∞}$ commuting with the ring~$R_d = k[m]$. In particular, it
commutes with the extension field~$K$. Moreover, the morphisms of pencils
in the IP1S sense are exactly the $R_d$-linear maps.
...
non-degenerate~\cite[VI~5.2]{lang} and there exists a unique element~$c ∈
K$ such that $\Tr (c z^i) = b(1, z^i)$ for all~$i=0, …, d-1$. We
immediately see that, for all~$x, y ∈ k$, $\Tr (c x y) = b(x, y)$.
...
of~$π^{m-1}$. We write~$τ$ instead of~$τ_m$ where there is no ambiguity.
We note that~$τ(xy)$, as a $k$-bilinear form on~$R_m$, is non-degenerate.
Actually, multiplication by~$π^{m-1}$ defines a (non-canonical)
...
orthogonal decomposition~$M = \bigperp M_d$, where $M_d$~is a finite free
...
Let~$k$ be a finite field where~$2 ≠ 0$; write~$R = k[π]/π^m$ and let~$M$
be a free $R$-module.
Then any regular $R$-bilinear form~$b$
on~$M$ is equivalent to one of the diagonal forms~$(1, …, 1)$ or~$(1, …,
d)$, where $Δ ∈ k^{×}$~is not a square.
\end{prop}
...
%>>>1
\section{Equivalence of pencils in characteristic two}%<<<1
In this section, we assume that the field~$k$ has characteristic two.
...
\subsection{The fully irregular part}%<<<2

We say that a bilinear pencil is \emph{fully irregular} if
its regular part, in the sense of Prop.~\ref{prop:kronecker}, is zero.
Any fully irregular bilinear pencil is isomorphic to an orthogonal sum of
...
compute an isomorphism, when it exists, between two fully irregular
...
\paragraph{Intrinsic Kronecker modules.}%<<<3
...
\paragraph{Kronecker modules with coefficients.}%<<<3
...
\paragraph{The automorphism group of a direct sum of Kronecker modules.}
...
\paragraph{Action on the diagonal coefficients.}
