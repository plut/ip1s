...
\begin{equation}
\def\arraystretch{.8}
\ro B = \mat{B_0\\-B_{∞} & B_0\\ & -B_{∞} & \smash{\ddots}\\
& & \smash{\ddots} \\ & & & B_0\\ & & & -B_{∞}}
\end{equation}
is injective. Since $\ro B$~has as a sub-matrix the matrix
\begin{equation}
\def\arraystretch{.8}
\ro C = \mat{C\\-CS & C\\ & -CS & \smash{\ddots}\\
& & \smash{\ddots} \\ & & & C\\ & & & -CS},
\end{equation}
...
block-diagonal matrix with its~$(d+1)$ diagonal blocks equal to~$C$;
therefore, the rank of~$C$ must be at least~$d$.
\end{proof}%>>>
