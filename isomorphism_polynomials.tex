...
$\dim E = h+1$, $\dim V/E^{⟂} = h$, and~$E ⊂ E^{⟂}$. We show
...
⨁ L_{X-μ,d,u'},&\text{if $p$~splits as~$∏_μ(X-μ)$ over~$k'$,}
\end{cases}
\end{equation}
...
polynomial. An analogous strategy for~$m ≥ 3$ would require a fine
...
can compute the first Kronecker block of~$B$. This algorithm is a direct
translation of part~\ref{SS:Kronecker-reduction}.
\newcounter{step}
\def\step#1{\paragraph{Step \arabic{step}: #1.}\stepcounter{step}}

\step{Compute a minimal isotropic vector~$(e_1,…,e_h)$}
...
\commentaire{XXX justifier pourquoi on s'en sort en~$O(n^3h)$ ? En
principe il faudrait utiliser le même algorithme qu'à l'étape
précédente...}
