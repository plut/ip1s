...
IP1S problem over a tower~$k'$ of real quadratic extension (a \emph{real
...
\nouveau{Two elements~$x$ and~$y$ of~$V$ are \emph{orthogonal} for~$b$
if~$b(x,y) = 0$; we write~$x ⟂ y$. The orthogonal of a sub-space~$W ⊂ V$
is~$W^{⟂}$.}
...
\nouveau{\begin{proof}
Let~$∑ λ^i e_i$ be a minimal isotropic vector, $h$~its degree, $E ⊂ V$
the sub-space spanned by the vectors~$e_i$, and~$E^{⟂}$ the orthogonal
of~$E$. By Prop.~\ref{prop:minimal-indep}, we know that $\dim E = h+1$,
$\dim V/E^{⟂} = h$, and~$E ⊂ E^{⟂}$. Morevoer, $(b_{λ})$~restricts to a
bilinear pencil on~$E^{⟂}/E$, whose minimal degree is at least~$≥ h$.
...
We first prove that there exists an orthogonal decomposition~$V =
V' ⊕ (E^{⟂}/E)$, where $V'$~is an extension of~$V/E^{⟂}$ by~$E$.
Such a decomposition amounts to a retraction~$u$ of the injection~$E ↪
E^{⟂}$ and a section~$v$ of the projection~$V → V/E^{⟂}$, such that for
all~$x ∈ E^{⟂}, y ∈ V/E^{⟂}$ and for all~$λ$, the relation~$b_{λ}(u(x),
v(y)) = b_{λ}(x, v(y))$ holds.
...
orthogonal decomposition~$V = V' ⊕ (E^{⟂}/E)$.

It remains to prove that $V'$~has a basis in which $(b_{λ})$~has
matrix~$K_{λ}$. This amounts to finding, when~$E = E^{⟂}$, a
retraction~$w$ of~$E ↪ V$ such that, for all~$x, y ∈ V$, $b_{λ} (x-w(x),
y-w(y)) = 0$, or in other words~$b_{λ} (x, y) = b_{λ} (x, w(y)) + b_{λ}
(w(x), y)$.
\end{proof}}
...
family~$(e_0,…, e_h, f_1, …, f_h)$ is free. For all~$i, j$, note that we have
