...
in worst-case time complexity~$O(n^4)$ in odd characteristic
and~$O(n^6)$ in characteristic two,
...
$b_{∞,R}$ is alternating. Since it is also regular,
it is hyperbolic. \todo{pourquoi}
...
In this section we assume that $q$~is a regular quadratic pencil
defined over~$K$, free of rank~$m$ over~$R = K[π]/π^e$.
By Proposition~\ref{prop:trace-local}, since $b_{∞}$ commutes with~$π$,
there exists a unique $R$-bilinear form~$b_{∞, R}$
such that~$b_{∞} = τ ∘ b_{∞, R}$;
moreover, since $b_{∞}$ and~$π b_{∞}$ are both alternating,
$b_{∞, R}$~is again alternating.
Therefore, $b_{∞, R}$ is \emph{hyperbolic}~\cite[I.(3.5)]{milnorhusemoller}:
namely, its rank over~$R$ is even, and there exists
a basis in which
\begin{equation}
b_{∞,R} ((x_1, …, x_r, x'_1, …, x'_r), (y_1, …, y'_r))
  \;=\; ∑_{i=1}^r x_i y'_i - x'_i y_i.
\end{equation}
The $k$-automorphisms of the polar pencil~$b_0 - λ b_{∞}$
are exactly the $R$-automorphisms of the alternating form~$b_{∞,R}$,
\emph{i.e.} the \emph{symplectic} automorphism of~$R^{2m}$.
...
% \begin{proof}[of Proposition~\ref{prop:primary}]%<<<
...
% we may apply Prop.~\ref{prop:trace-ext} even when it is not a field.
