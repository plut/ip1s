...
\begin{abstract}%<<<
In this paper, we study the Isomorphism of Polynomial with Two Secrets
(IP2S) problem for two quadratic polynomials with $n$ variables over a
finite field of odd characteristic: given two homogeneous quadratic
polynomials $(\bm{a},\bm{b})$ on $n$ variables, we compute two bijective
linear maps $(s,t)$ such that $t \circ \bm{a} \circ s = \bm{b}$. We show
an algorithm that allows to recover $s$ and $t$ in $O(n^3)$.
This problem has been introduced in cryptography by Patarin back in 1996.
The special case of this problem where~$t=1$ is called
the IP1S problem. At ASIACRYPT 2013, Macario-Rat, Pl\^ut and Gilbert
explained how generic solvers (for example using Gr\"obner bases) may
solve the IP1S problem, by describing a polynomial-time
algorithm solving ``cyclic instances'' of this problem using pencils of
quadratic forms over all finite fields. They showed that this problem is
...
The Isomorphism of polynomial problem has been introduced by Patarin
in~\cite{DBLP:conf/eurocrypt/Patarin96} to build an efficient
authentication scheme as an alternative to the Graph Isomorphism Problem
used by 
\commentaire{??}.

\commentaire{parler du problem IP Patarin et lien avec la crypto}
\commentaire{Cas difficile $m=2$}

\subsection*{Previous Work}

\commentaire{0. parler des cas Gröbner.}

\commentaire{1. IP1S asiacrypt.}

In \cite{MPG2013}, Macario-Rat, Plût and Gilbert explained how to
solve cyclic instances of the IP1S problem over finite fields of any
...
\subsection*{Mathematical Background}%<<<2
