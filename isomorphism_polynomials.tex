...
From Props.~\ref{prop:kronecker-split} and~\ref{prop:kronecker-matrix} we
...
degenerate. However, the decomposition given by Lemma~\ref{lem:finite}
below still applies.

\begin{prop}\label{lem:finite}
Let~$\bm{b}$ be a regular symmetric pencil on~$V$. For any two vectors
...
(ii) We have to prove the following: for any vector~$x$, $b_{∞}(x, W') =
...
(λ-m)$ where~$a ∈ \ro C(m)^{×}$.
\item \label{it:sym-adj} Let~$b_{λ} = ta(λ-m)$ be a finite pencil and~$s ∈ \ro C(m)$.
...
= tam$. Point~\ref{it:sym-form} follows directly from~\ref{it:sym-comm},
and \ref{it:sym-adj} is a straightforward computation.
\end{proof}

Point~\ref{it:sym-adj} of Lemma~\ref{lem:structure-sym} translates to the
following result.
...
\begin{proof}
Points~\ref{it:rel-J} and~\ref{it:Juw} are straightforward computations.
Point~\ref{it:commute-H} follows from the linear relations on the
coefficients on a matrix~$A$ such that~$H_u A = A H_v$.
\end{proof}
%>>>

We now turn to the general case where $b_{λ}$~is a local pencil. Let~$m$
be its characteristic endomorphism~$m$ and $p^d$ be its characteristic
polynomial, where $p$~is an irreducible polynomial of degree~$e$. We
describe the matrix~$M$ of~$m$ in an appropriate basis of~$V$.

Let~$M_p$ be the companion matrix of~$p$; then $k[M_p]$~is isomorphic to
the extension field~$K = k[μ]/(p(μ))$.
...
blocks~$M_{n_i} = (H_{n_i} + μ)^{♭}$. In this particular case, the
...
integers~$e_{i,j}$ satisfy the relations:
...
the following. Let~$D$ be the diagonal matrix with entries~$(H^{n_1-n_i})
∈ R^{r×r}$. For any~$A = (a_{i,j}∈ \ro C(M)$, let
\begin{equation}
...
R$ having a non-square image~$δ$ in~$R$. Any regular symmetric matrix
...
Prop.~\ref{prop:local-diag}. We also note that, if $(b_{λ})$~is regular,
then~$h = 0$ and the algorithm in this case has complexity~$\Ot(n^3)$.
