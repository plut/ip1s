...
We shall say that a pencil in this form \emph{reduced}, and call the
...
\end{equation}
where~$x_{d-2i} = 0$ for~$d-2i < 0$. This has a solution for all~$i$
except for~$i = m$, where the equation depends on~$x_{d-1-2m}^2 +
x^{}_{d-1-2m}$. Hence, we may assume that~$\alpha_{d-1-m}$ is either~$0$
or~$\delta$, and all others~$\alpha_{i}$ are zero.
...
Let~$A$ be a cyclic pencil of quadratic forms, reduced
in the sense of Prop.~\ref{prop:quad-can}.
\begin{enumerate}
\item Assume that~$\alpha_1 \neq 0$. Then group of linear
transformations~$g$ fixing the characteristics~$\alpha_1$ and~$\alpha_2$
is isomorphic to~$\K[M] \times \F_2$.
\item Assume that
...
u \in \K[M]$. Then the characteristics~$\alpha_1$ and~$\alpha_2$ are
...
\end{equation}
For any~$x$, the first equation determines~$z$ such that~$(1+z)$~is
invertible. With~$\alpha_1 = H^m$, the second equation then has exactly
two solutions~$y$ and~$y + H^{d-1-2m}$.
...
solutions.
\end{proof}
% The solutions~$y$ of the equation~\eqref{eq:aut-G2} form an additive
% group isomorphic to~$\F_2 \times \L^m$.
% 
% 
% \begin{proof} The automorphisms of~$A_\infty$ are of the form~$G_2(y)
% G_3(u^{-1}) G_1(x)$, with the conditions \begin{equation} (u+1)\:
% \alpha_1 = \alpha_2 \,\varphi(x) + \theta(x), \qquad (u+1)\: \alpha_2 =
% \alpha_1 \,\varphi(y) + \theta(y). \end{equation} Write~$\alpha_1 =
% H^m$ and~$\alpha_2 = 0$ or~$\alpha_2 = H^{d-1}$. If~$d \geq 2$
% or~$\alpha_2 = 0$ then we have~$\alpha_2^2 = \alpha_2 \theta(x) = 0 \in
% \K[M]$. In this case the first equation has a unique solution~$u = u(x)
% \in \K[M]^{\times}$ and the second one reduces to to~\eqref{eq:aut-G2}.
% In the case where~$d = 1$ and~$\alpha_2 = \delta \notin \wp(\L)$, the
% first equation solves as~$u = \delta x^2 + x + 1$, which is always
% non-zero in~$\L$, and the second one is~$y^2 + y = \wp(\delta x)$,
% which has the two solutions~$y = \delta x$ and~$y = \delta x + 1$.\qed
% \end{proof}
