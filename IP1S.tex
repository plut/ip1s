...
\begin{align*} \numberthis
X &= G_2(X_1^{-1} X_3) \:\cdot\: G_3(X_1) \:\cdot\: G_1(X_1^{-1} X_2) \\
  &= G_2(X_2^{-1} X_4) \:\cdot\: G_3(X_2) \:\cdot\: G_1(X_2^{-1} X_1)
  \:\cdot\: G_4
  \qquad \qquad \qed \\
\end{align*}
\end{proof}
...
$v_H(X)$ is the unique integer such that~$X = H^{v_H(X)} U$ with
$U$~invertible.
...
matrices whose values~$\alpha_{i} = \psi(A_i)$ are such that~$(\alpha_1,
\alpha_2)$ are either~$(0, 0)$, $(H^m, 0)$ for~$0 \leq m \leq (d-1)/2$,
or~$(1, \delta H^{d-1})$. Moreover, these cases are mutually exclusive.
\end{proposition}
...
\begin{align*}
%\label{eq:Gi-alpha}
G_1(X):&\quad\alpha_{1} \leftarrow \alpha_1 + \varphi(X)\, \alpha_2\,  +
...
\numberthis
G_2(X):&\quad\alpha_{2} \leftarrow \alpha_2 + \varphi(X)\, \alpha_1\,  +
...
\end{align*}
We note that the equations for~$G_1$ are diagonal in the
coefficients~$x_i$, except for the last term; therefore, these equations
may be solved modulo~$H^{d-1}$ in the same way as~$\alpha_1 \leftarrow
\alpha_1 + \varphi(X) \alpha_2$. The same applies for~$G_2$; therefore,
the orbit of~$(\alpha_1, \alpha_2)$ (modulo~$H^{d-1}$)
under~$O(\Polm{A})$ is the same as its orbit under~$SL_2(\K[M])$.
Let~$m \leq d$ such that~$(\alpha_1, \alpha_2)$ generate the
ideal~$(H^m)$ of~$\K[M]$; then we may assume that~$\alpha_1 \equiv H^m$
and~$\alpha_2 \equiv 0 \pmod{H^{d-1}}$.

Using a transformation of the form~$G_1(x H^{d-1})$, we may assume
that~$\alpha_1 = H^m$. If~$m \geq 1$, then a suitable transformation of
the form~$G_2(y H^{d-1})$ gives~$\alpha_2 = 0$; else~$m = 0$ and
the same transformation replaces
\begin{equation}\label{eq:G2-Arf}
\alpha_2 \leftarrow \alpha_2 + (y^2 + y) H^{d-1}.
\end{equation}
Hence, we may assume that either~$\alpha_2 = 0$ or~$\alpha_2 = \delta
H^{d-1}$.
...
to replace~$\alpha_1$ with~$0$ in this case.
\end{proof}

In the generic case, the elements~$(\alpha_1, \alpha_2)$ generate the
unit ideal of~$\K[M]$, so that we have~$\alpha_1 = 1$. The image
modulo~$\wp(\L)$ of the coefficient~$\alpha_{2, d-1}$ is known as the
\emph{Arf invariant}~\cite{milnor1973symmetric} of~$A_{\infty}$.

\begin{proposition}\label{prop:aut-can}
The automorphisms of the canonical form of Prop.~\ref{prop:quad-can}
are
\begin{enumerate}
\item if~$\alpha_1 = 0$: the full orthogonal
group~$O(\Polm{A})$;
\item if~$\alpha_1 = H^m$ with~$m \leq (d-1)/2$: generated by the
transformations
\begin{align}
\label{eq:aut-G1}
G_3(u(x))\:\cdot\: G_1(x),
& \quad\text{for~$x \in \K[M]$ and some function~$u: \K[M] \rightarrow
\K[M]^{\times}$;}\\
\label{eq:aut-G2}
G_2(y), &\quad \text{for~$y \in \K[M]$ such that $\theta(y) = H^m\varphi(y)$.}
\end{align}
\end{enumerate}
\end{proposition}

The solutions~$y$ of the equation~\eqref{eq:aut-G2} form an
additive group isomorphic to~$\F_2 \times \L^m$.


\begin{proof}
The automorphisms of~$A_\infty$ are of the form~$G_2(y) G_3(u^{-1})
G_1(x)$, with the conditions 
\begin{equation}
(u+1)\: \alpha_1 = \alpha_2 \,\varphi(x) + \theta(x), \qquad 
(u+1)\: \alpha_2 = \alpha_1 \,\varphi(y) + \theta(y).
\end{equation}
Write~$\alpha_1 = H^m$ and~$\alpha_2 = 0$ or~$\alpha_2 = H^{d-1}$.
If~$d \geq 2$ or~$\alpha_2 = 0$ then we have~$\alpha_2^2 = \alpha_2
\theta(x) = 0 \in \K[M]$. In this case the first equation has a unique
solution~$u = u(x) \in \K[M]^{\times}$ and the second one reduces to
to~\eqref{eq:aut-G2}. In the case where~$d = 1$ and~$\alpha_2 = \delta
\notin \wp(\L)$, the first equation solves as~$u = \delta x^2 + x + 1$,
which is always non-zero in~$\L$, and the second one is~$y^2 + y =
\wp(\delta x)$, which has the two solutions~$y = \delta x$ and~$y =
\delta x + 1$.\qed
\end{proof}
...
% 
% $\alpha_1=H^m$, $\deg(\alpha_1+\alpha_3)\gt m$, $\alpha_2=0$ or $\alpha_2=\delta H^{d-1-m}$, $\deg(\alpha_4)\geq m$,
% for some $m \in
% \acco {0, \dots, d}$,
% and 
% some fixed
...
% either
% $\alpha_1=H^m$ or $\alpha_3=H^m$, $\deg(\alpha_1+\alpha_3)= m$, $\alpha_2=\alpha_4$, $\deg(\alpha_2)\gt m$ 
% for some $m \in
% \acco {0, \dots, d}$,
% and 
% some fixed
...
% 
% \begin{proof}
% By Prop.~\ref{prop:bil-can}, we may compute bases in which the pencils of polar forms
% have the form~\eqref{eq:bil-can}. In the same bases the pencils have the form~\eqref{eq:quad-can} with~$M$, $T$, $M_0$, $T_0$ as in
% Theorem~\ref{transpose-symmetric} and where~$A_i$ are some diagonal matrices.
...
% use the transformations~$G_i(X)$ from~\eqref{eq:orthogonal-gen}. Let~$X =
% x_0 + \dots + x_{d-1} H^{d-1} \in \K[M]$. We define
% \begin{equation}\label{eq:theta}
% \theta(X) = \psi (\Diagm{T\,X}) = \sum_{2i \geq (d-1)} x_{2i-(d-1)} H^i.
% \end{equation}
% The effects of the elementary transformations~$G_i(X)$ on the
...
% % \quad\alpha_3 \leftrightarrow \alpha_4.
% \end{align*}
% and the transformations act in the same way on the pair~$(\alpha_3,
% \alpha_4)$.
% 
% As in Prop.~\ref{prop:diagonal-isomorphism}, we write~$A_i$ as
% $d$~blocks~$A_{i,j}$ and define~$\alpha_{i,j} = \psi_0{A_{i,j}}$.
% From what we get above we explicit the effects of the elementary transformation~$G_1(X)$ on the
% coefficients~$\alpha_{i,j}$:
% \begin{align*}
% %\label{eq:Gi-alphai}
% G_1(X):&\quad\alpha_{1,m} \leftarrow \alpha_{1,m}
%   + \sum_{i+j=m} \alpha_{2,i}^{} x_j^2 \quad \text {for}\, m\lt \frac{d-1}{2},\\
% &\quad\alpha_{1,m} \leftarrow \alpha_{1,m}
%   + \sum_{i+j=m} \alpha_{2,i}^{} x_j^2 + x_{2m-(d-1)} \quad \text {for}\, m\geq \frac{d-1}{2};\\
% 	\end{align*}
% 
% If all $\alpha_i=0$, we are done: the pencil is canonical. If not, we search the value $\alpha_i$ with smallest degree. Using $G_4$, we may assume it is $\alpha_1$ or $\alpha_3$. We first suppose that we have $\deg(\alpha_1+\alpha_3)\gt m$, that is $\alpha_1$ and $\alpha_3$ have the same trailing term.
% Using $G_3$, we may assume $\alpha_1=H^m$, and therefore $\alpha_3=H^m+\alpha$, with $\deg(\alpha)\gt m$.  We look then for $X$ such that $G_2(X)(\alpha_2)=0$. We suppose first that $m\lt \frac{d-1}{2}$. In this case the system is triangular and all equations can be solved except maybe for this one: $\alpha_{2,d-1-m} = x_{(d-1)-2m}^2 + x_{(d-1)-2m}.$ Therefore we may assume that $\alpha_2=0$ or $\alpha_2=\delta H^{d-1-m}$ for some fixed
% $\delta \in \K[M_0] \smallsetminus \wp(\K[M_0])$. We note also that since $m\lt \frac{d-1}{2}$, $G_2(X)$ does not decrease the degree of $\alpha_4$.
% We suppose now that $m\geq \frac{d-1}{2}$. In this case, the system $G_2(X)(\alpha_2)=0$ has many solutions, but we can still find one such that $x_{2i-(d-1)}=0$ for $i\lt m$, that is one that does not decrease the degree of $\alpha_4$. So in this case, we can also assume that $\alpha_2=0$ and $\deg(\alpha_4)\geq m$.
% 
% We  now examine the case where $\deg(\alpha_1 + \alpha_3)=m$. Using again $G_3$, we may assume that $\alpha_1=H^m$ or $\alpha_3=H^m$. Let's note $\alpha_1+\alpha_3=H^{m}\alpha$ where $\alpha$ is invertible. We are looking for $X$ such that $G_2(X)(\alpha_2)=G_2(X)(\alpha_4)$. By hypothesis on the degree, we can write $\alpha_2+\alpha_4=H^m \alpha'$ for some $\alpha'$. We naturally choose $X=\varphi^{-1}(\alpha'\alpha^{-1})$.
% At this stage, we can consider that $\alpha_2=\alpha_4$. However, the condition on the degree may not hold. If by chance $\deg(\alpha_2)\gt m$, then we are done. If on the contrary $\deg(\alpha_2)\leq m$, then by using $G_4$, we search instead for a canonical form of the first kind. \qed
% 
	%
...
to the form of Prop.~\ref{prop:quad-can}. Both pencils then have the same
values~$\alpha_1$ and~$\alpha_2$; it remains to find an automorphism
relating the values~$\alpha_3$ and~$\alpha_4$. This can be done via
Prop.~\ref{prop:aut-can}. Following along the proof of both propositions,
we see that they are constructive and that all linear algebra algorithms
used require at most~$\Otilde{n^3}$ field operations.

Solutions of the IP1S problem correspond bijectively to automorphisms of
the pencil. Without loss of genericity, we may assume that~$\alpha_1 = 1$
and $\alpha_4$~is invertible in~$\K[M]$. According to
Prop.~\ref{prop:aut-can}, automorphisms of such a pencil are of the
form~$G_2(y) G_3(u^{-1}) G_1(x)$ where
\begin{gather*}\label{eq:aut-generic}
u+1 \;=\; \theta(x) + \alpha_2\, \varphi(x), \quad
  (u+1)\: \alpha_2 \;=\; \theta(y) + \varphi(y),\\
(u+1)\: \alpha_3 \;=\; \theta(x) + \alpha_4\, \varphi(x), \quad
  (u+1)\: \alpha_4 \;=\; \theta(y) + \alpha_3\, \varphi(y).
\end{gather*}
Eliminating~$(u+1)$ from the first two equations gives~$\varphi(y) =
\theta(y)$, which means that~$y = 0$ or~$y = H^{d-1}$. Each of these
cases has exactly one solution in~$x$ and~$u$. Therefore, the pencil
generally has exactly one non-trivial automorphism.
\end{proof}
