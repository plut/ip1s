...
characteristic-two analogue of the set of squares. For any element
$\alpha$ of $\K[M]$, we call {\em degree} of $\alpha$ that we note
$\deg(\alpha)$ the smallest integer $m$ such that there exist an
invertible $\alpha'$ of $\K[M]$ such that $\alpha=H^m\alpha'$.
...
coefficients~$\alpha_{i}$ are:
\begin{align*}
%\label{eq:Gi-alpha}
G_1(X):&\quad\alpha_{1} \leftarrow \alpha_1 + \varphi(X)\, \alpha_2\,  + \psi(\Delta(TX)),\\
...
G_2(X):&\quad\alpha_{2} \leftarrow \alpha_2 + \varphi(X)\, \alpha_1\,  + \psi(\Delta(TX)),\\
...
G_4:&\quad\alpha_1 \leftrightarrow \alpha_2, \quad\alpha_3 \leftrightarrow \alpha_4.
\end{align*}

A direct computation gives \[\psi(\Delta(TX))= \sum_{i\geq \frac{d-1}{2}}{x_{2i-(d-1)}H^i}.\].
