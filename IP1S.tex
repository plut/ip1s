...
group~\cite{dieudonne-dickson}. These considerations of sign are used
only for counting the solutions to the IP1S problem.
...
matrices such that the values~$\alpha_{i} = \psi(A_i)$ satisfy
\begin{enumerate}
\item $\alpha_1 = \alpha_2 \in \acco{0, \delta H^{d-1}}$, where
$\delta$~is a fixed element of $\K[M_0] \smallsetminus \wp(\K[M_0])$;
\item $\alpha_3 = H^m$ for an integer~$m \in \acco {0, \dots, d}$.
\end{enumerate}
...
The value~$\alpha_1 = \in \K / \wp(\K)$ is also known as the Arf
invariant of~$A_\infty$~\cite{milnor1973symmetric}.

\begin{proof}
By Prop.~\ref{prop:bil-can}, we may compute bases in which the
pencils have the form~\eqref{eq:quad-can} with~$M$, $T$, $M_0$, $T_0$
as in Theorem~\ref{transpose-symmetric} and~$A_i$ are some symmetric
matrices. We now perform elementary transformations of the orthogonal
group of~$\Polm{A}$ to simplify the diagonal part of the quadratic
pencil. We use the (positive) transformations~$G_1(X)$ and~$G_2(X)$
from~\eqref{eq:orthogonal-gen} for a matrix~$X = x_0 + \dots + x_{d-1}
H^{d-1} \in \K[M]$.

As in Prop.~\ref{prop:diagonal-isomorphism}, we write~$A_i$ as
$d$~blocks~$A_{i,j}$ and define~$\alpha_{i,j} = \psi_0{A_{i,j}}$.
The effect of the elementary transformation~$G_1(X)$ on the
coefficients~$\alpha_{i,j}$ is:
\begin{equation}\label{eq:alpha12}
\alpha_{1,m} \leftarrow \alpha_{1,m}
+ \sum_{i+j=m} \alpha_{2,j}^{} x_j^2 + x_{2m-(d-1)};
\end{equation}
The first $(d-1)$~equations of this system are triangular with pivot
equal to the first non-zero of the coefficients~$\alpha_{2,j}$,
say~$\alpha_{2,m}$. Using formulas~\eqref{eq:alpha12}, we may assume
that all~$\alpha_{1,j}$ are zero for~$j = m, \dots, d-2$. Likewise, the
elementary transformations~$G_2(X)$ allows us to cancel the first~$(d-1)$
entries of~$A_2$. The last equation of~\eqref{eq:alpha12} then reads as
$\alpha_{1,d} \leftarrow \alpha_{1,d}^{} + \alpha_{2,d}^{} x_{d}^2 +
x_{d}$; this allows to change both~$\alpha_{1,d}$ and~$\alpha_{2,d}$ by
elements of~$\wp(\K)$, hence the result.

\medskip

The only remaining automorphisms of a pencil in this form are the
elementary transformations~$G_3(X) = \smat{X}{0}{0}{X^{-1}}$, with~$X \in
\K[M]$ invertible. Write~$\alpha_i = \psi(A_i) \in \K[M]$.
By~\eqref{eq:psi}, the effect of~$G_3(X)$ on~$\alpha_3$ and~$\alpha_4$ is
\begin{equation}\label{eq:alpha34}
\alpha_3 \leftarrow \alpha_3 \;\varphi(X), \quad
\alpha_4 \leftarrow \alpha_4 \;\varphi(X)^{-1}.
\end{equation}
By choosing an appropriate unit~$X \in \K[M]$, we may assume
that~$\alpha_3 = H^m$ for some~$m$ (with~$m = d$ when~$\alpha_3 = 0$).
% The last degree of freedom is then choosing~$X$ such that~$\varphi(X)
% \alpha_3 = \alpha_3 = H^m$, which means that~$X \in 1 + H^{d-m}
% \K[M]$.\qed
\end{proof}
...
the canonical pencil. In general, half the automorphisms are positive; in
addition to the computations of Prop.~\ref{prop:quad-can}, we note that
the negative automorphism~$G_4$ swaps the values of~$\alpha_3$
and~$\alpha_4$.

In the generic case, the canonical pencil is such that~$\alpha_3$
and~$\alpha_4$ are both invertible in~$\K[M]$. This pencil has therefore
only two automorphisms: the identity and the (negative) automorphism~$G_4
\circ G_3(\varphi^{-1}(\alpha_4))$. The IP1S problem for pencils
congruent to this canonical pencil has therefore exactly two solutions.
\end{proof}
