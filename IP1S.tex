...
form~$b = \Pol(q)$ defined by $b(x, y) = q(x+y) - q(x) - q(y)$. This is a
...
q(x)$;
\item the polar form~$\Pol(q)$ is bilinear.
\end{enumerate}
...
From the definition of~$b = \Pol(q)$ we derive the \emph{polarity
identity}
...
A_{\lambda}$~is non-zero and the associated projective pencil is regular.
Therefore, it has a basis that provides a regular affine pencil; this
basis is defined over an extension of the field~$\K$, and over~$\K$
itself if it is infinite.
...
%\label{OUR}
%
\subsubsection{Similarity of a matrix and its transposed.}

The next result is intensively used in the sequel to deal with symmetric  pencils.
\begin{theorem}
...
$n\times n$ matrices $\Gamma(p)$ (also known as the companion matrix of the polynomial $p$) and $\Theta(p)$ as:
\begin{equation}
\label{eq:companion}
...
\end{pmatrix}
\end{equation}
One can easily verify that for any polynomial $p$, $M=\Gamma(p)$ and $T=\Theta(p)$
...
In this case,
the polarity identity~\eqref{eq:polarity} identifies quadratic forms with symmetric
bilinear forms, or again with symmetric matrices with entries in~$\K$.
We shall therefore write a quadratic pencil~$A$ as~$A_{\lambda} = A_0 +
\lambda A_{\infty}$, where $A_0$~and~$A_{\infty}$ are symmetric matrices.
...
pencil congruence problem are exactly the invertible~$X$ commuting
with~$M $ and such that~$\transpose{X} A_{\infty} X = B_{\infty}$.
\end{enumerate}
\end{proposition}
...
$\transpose{X} A_{\infty} (\lambda + M ) X \;=\;
\transpose{X} A_{\infty} X (\lambda + X^{-1} M  X)$.
\end{proof}
...
B_0$, with a complexity of~$\Otilde{n^3}$ operations. Expressing
$C = A_{\infty}^{-1} B_{\infty}$ as a polynomial in~$M$ can be done by
computing the minimal polynomial~$f$ of~$C$ and then finding the correct
root~$c = g(M)$ of~$f$ in~$\K[M ]$, again in $\Otilde{n^3}$.

The minimal polynomial of~$M$ is~$p^e$, where $p$~is irreducible. A
square root~$r$ of~$g(M)$ in~$\K[M ]$ is a Hensel lift of a square root
of~$g$ modulo~$p$, which in turn is a square root of~$g(x)$ in the
extension field~$\K[x] / p(x)$ and hence computable with Cipolla's
algorithm in $\Otilde{n^3}$. \end{proof}
...
\XXX{Conflit sur 2 prochains paragraphes}

We note that cyclic pencils are generic both in an algebraic-geometric
and in a numeric sense. Therefore, random instances of the IP1S problem
in odd characteristic are easily solvable. Moreover, in the non-cyclic
case, since the commutant~$\commute{M}$ is strictly larger than~$\K[M]$,
the number of solutions of the IP1S instance can be large. For example,
in the extreme non-cyclic case where~$M = 1$, the commutant is the whole
space of matrices and hence solutions of IP1S correspond bijectively to
elements of the orthogonal group~$O(A_{\infty})$, therefore the number of
solutions is~$O(q^{2n^2-n})$ where $q$~is the number of elements of~$\K$
and $n$~is the dimension of the problem.

Finally, we may conclude that in this first case, the considered sub-system has exactly two solutions that can be computed in polynomial time. 
...
An overview of our solving approach is given in Fig.~1 in Appendix~\ref{app:figs}.
...
\XXX{Conflit: version de Jérôme}
%\iffalse
This section deals with the IP1S problem for pencils of quadratic forms
in characteristic~two.

\subsubsection{Basic facts about quadratic forms in characteristic two}
...
\Pol(q)$ attached to a quadratic form~$q$ is an alternating bilinear
form.
...
\subsubsection{Pencils of alternating bilinear forms}

Although quadratic forms only produce alternating bilinear forms in
characteristic two, the following two lemmas about alternating forms are
true in all characteristics.

\begin{lemma}\label{lem:alt-bezout}
Let~$b$ be a pencil of alternating bilinear forms on~$V$ with
characteristic endomorphism~$f$.
Let~$p, q \in \K[t]$ be two mutually prime polynomials. Assume that~$x, y \in
V$ satisfy~$p(f) (x) = 0$ and~$q(f)(y) = 0$. Then, for all~$b_{\lambda}$ in the
pencil~$b$,
\[ b_{\lambda} (x, y) = 0. \]
\end{lemma}

\begin{proof}
Let~$u, v$ be such that $u p + v q = 1$. Note that, for all~$x, y \in V$,
we have~$b_{\infty} (x, fy) = b_0 (x, y) = b_{\infty} (fx, y)$;
therefore, all elements of~$\K[f]$ are self-adjoint with respect
to~$b_{\infty}$. From this we derive the following:
\begin{equation}
\begin{split}
...
Lemma~\ref{lem:alt-bezout}, we may replace~$V$ by one of the primary
...
Let~$a(t) = 1 + a_1 t + \dots + a_{n-1} t^{n-1}$ be a polynomial
and~$x' = a(f) x$. Then we still have~$b_{\infty}(x', f^{n-1} y) = 1$, and
moreover we can choose~$a$ so that~$b_{\infty}(x', f^i y) = 0$ for all~$i = 0,
\dots, n-2$. Then $(x', f x', \dots, f^{n-1} x', \allowbreak
f^{n-1} y, f^{n-2} y, \dots, fy, y)$ is a symplectic basis for~$b_\infty$
on~$W$ and its Lagrangian is~$\K[f] x$, which is obviously stable by the
characteristic endomorphism.
\end{proof}

In the basis constructed by Lemma~\ref{lem:alt-symplectic}, the
pencil~$b$ has a block-matrix decomposition of the form
\begin{equation}
b_\infty = \mat{0 & 1 \\ 1 & 0}, \quad
b_0 = \mat{0 & \transpose{g} \\ g & 0}; \qquad
f = b_{\infty}^{-1} b_0 = \mat{g & 0 \\ 0 & \transpose{g}}.
\end{equation}
The matrix~$g$ is called the \emph{Pfaffian matrix} of~$b$.
The proof of the lemma actually builds a symplectic basis in which the
Pfaffian matrix is in rational (Frobenius) normal form.

\subsubsection{Pencils of quadratic forms}

The following theorem is a direct consequence of
Lemma~\ref{lem:alt-symplectic}.

\begin{theorem}\label{thm:quad-can}
...
Q_{\infty} = \mat{A_1 & T \\ 0 & A_2}, \quad
Q_0 = \mat {A_3 & TM \\ 0 & A_4},
\end{equation}
where $T$, $TM$~are symmetric and $A_1, A_2, A_3, A_4$~are diagonal
matrices.
\end{theorem}

\begin{proof}
\begingroup \let\s\scriptstyle
\def\sm#1#2#3#4{\mat{\s#1&\s#2\\\s#3&\s#4}}
\def\arraystretch{.6}
By Lemma~\ref{lem:alt-symplectic}, we may assume that the polar pencil
of~$q$ has matrices $B_{\infty} = \sm{0}{1}{1}{0}, 
B_0 = \sm{0}{\transpose{M}}{M}{0}$. By
Theorem~\ref{transpose-symmetric}, there exists a symmetric matrix~$T$
such that~$TM$~is again symmetric. Let~$P = \sm{1}{0}{0}{T}$.
Then we have~$\transpose{P} B_{\infty} P = \sm{0}{T}{T}{0}$ 
and $\transpose{P} B_{0} P = \sm{0}{TM}{TM}{0}$. When also taking account
of the diagonal coefficients of the pencil, the theorem immediately
follows.
\endgroup
\end{proof}

\XXX{Conflit: version de Gilles}
...
{ a & \dot{A} & & \dot{B} & b \\
& LM_a & L & LM_b & \\ };
%\draw[double,double distance=5pt] (m-1-1) ï¿½ (m-1-3);
...
%%\end{pmatrix}.
%%$
%%[dire un mot peut-ï¿½tre sur $\lambda I + M'$ et $\lambda T + TM'$ comme en caracï¿½ristique impaire ?]
