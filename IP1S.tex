...
\begin{proposition}\label{prop:det-solutions}
Let~$A, A'$ be two congruent cyclic pencils of quadratic forms with
characteristic values~$(\alpha_i), (\alpha'_i)$. Assume that
\begin{equation}\label{eq:determinant}
\alpha'_1\, \alpha_4 \:+\: \alpha_2\, \alpha'_3 \;\not \equiv\; 0 \pmod{H}.
\end{equation}
Then there are at most four isomorphisms between~$A$ and~$A'$, and
computing one of them is possible using no more than~$\Otilde{n^3}$ field
operations.
\end{proposition}

\begin{proof}
By Prop.~\ref{prop:quad-can}, we may assume that~$A, A'$~are reduced.
Without changing the values~$\alpha_1, \alpha_2$, we may further assume
the divisibility properties: $\alpha'_1 \mid \alpha_2$, $\alpha'_1 \mid
\alpha'_3 \mid \alpha_4$. Let~$\beta_i \in \L[H]$ be such that
\begin{equation}\label{eq:beta}
\alpha_2 = \alpha'_1 \beta_2, \qquad
\alpha_3 = \alpha'_1 \beta'_3, \qquad
\alpha_4 = \alpha'_3 \beta_4 = \alpha'_1 \beta'_3 \beta_4.
\end{equation}
...
always determines an invertible~$u \pmod{H^{d-v_H (\alpha'_1)}}$.
Write~$X = \smat{x \\ y}$ and $\Delta = \beta_2 + \beta_4$.
Eliminating~$u$ from the last three equations and performing some
elementary line operations yields
\begin{equation}\label{eq:matrix2}
\alpha'_1 \mat{\beta_2^2 & 1 \\ \beta'_3 \Delta & 0 \\ 0 & 0}
  \varphi (X)
+ \mat{\beta_2 & 1 \\ \beta'_3 & 0 \\ \beta'_3 \beta_4; & \beta'_3}
  \theta (X)
+ \mat{0 & 0 \\ 1 & 0 \\ \beta_2 & 1} \theta (MX) + C = 0.
\end{equation}
for some column vector~$C$. Let~$Y = \smat{1 & 0 \\ \varphi^{-1}
(\beta_2^2) & 1} X$. Using the relation
\begin{equation}\label{eq:phi-theta}
\varphi(a) \, \theta(x) \;=\; \theta (a^2 x)
\qquad \text{for all~$a, x \in \L[H]$},
\end{equation}
the equations from \eqref{eq:matrix2}~separate into
\begin{equation}\label{eq:matrix3}
\alpha'_1 \mat{0 & 1 \\ \beta'_3 \Delta & 0 \\ 0 & 0} \varphi(Y)
+ \mat{0 & 1 \\ \beta'_3 & 0\\ \beta'_3 \Delta & \beta'_3} \theta(Y)
+ \mat{0 & 0 \\ 1 & 0 \\ 0 & 1} \theta(MY)
+ C \;=\; 0.
\end{equation}
The only non-zero minor of the first matrix is~$\alpha'_1 \beta'_3 \Delta
= \alpha'_1 \alpha_4 + \alpha_2 \alpha'_3$. If it is invertible, then
this matrix is injective, and therefore~$Y$ is solution of an equation of
the form
\begin{equation}\label{eq:contracting}
Y = F(Y) = 
B_0\,  \theta \circ \varphi^{-1} (Y) +
B_1\,  \theta \circ \varphi^{-1} (MY) +
C,
\end{equation}
where the map $F \pmod{H^{d-1}}$ is contracting for the valuation~$v_H$.
Therefore, this equation has a unique solution~$Y \pmod{H^{d-1}}$.
Solving for the coefficients of~$H^{d-1}$ in~$Y$ yields two
Artin-Schreier equations and one linear equation; between them, these
equations have at most four solutions.
\end{proof}
