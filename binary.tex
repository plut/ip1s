...
vector space over~$K$. Let~$b: V ⊗ V → k$ be a $k$-bilinear form
commuting with~$K$. Then there exists a unique $K$-bilinear
form~$b_K: V ⊗ V → K$ such that~$b = \Tr_{K/k} \,∘ \,b_K$.
\end{prop}


\begin{proof}
We first recall the (classical) proof of result when~$V = K$. In this
case, let~$z$ be a primitive element of~$K$, $d = [K:k]$. Since~$K/k$ is
...
Now let~$n = \dim_K V$ and write the matrix of~$b$ as~$(b_{i,j})$,
where $b_{i,j}: K ⊗ K → k$ is $k$-bilinear. Since $b_{i,j}$ again
satisfies the hypotheses of the proposition, there exists a
unique~$c_{i,j} ∈ K$ such that~$b_{i,j} (x,y) = \Tr_{K/k} (c_{i,j} x y)$.
The bilinear form~$b_K$ is then the form with matrix~$(c_{i,j})$.
\end{proof}
