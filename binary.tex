...
\transpose{α_{i,d}} · β_{i,d'} ∈ E_d^{∨} ⊗ E_{d'}^{∨} ⊗ H_{d'-d}$ is the standard
...
isotropic on~$⨁ E_d ⊗ H_{d-1}$ and on~$⨁ E_d ⊗ H_d^{∨}$, for any
...
H_{d}^{∨}$ is a $σ$-linear form. The quadratic form~$q$ is determined by the
restrictions of~$σ^{-1} ∘ q$ to these spaces, which are linear forms.
For~$u ∈ E_d$, $f ∈ H_{d-1}$ and~$φ ∈ H_d^{∨}$, we write $λ_d(u ⊗ f) =
σ^{-1} ∘ q\: (u ⊗ f)$ and~$μ_d (u ⊗ φ) = σ^{-1} ∘ q\: (u ⊗ φ)$.
% which we write~$λ_d ∈ E_d ⊗
% H_{d-1}^{∨}$ and~$μ_d ∈ E_d ⊗ H_d$ (using the scalar product to
...
q ∘ F\;(u ⊗ f) &= ∑_i q(α_{i,d} (u) f) + 
  ∑_i q(f^{∨} γ_{i,d} (u)) + ∑_i b(γ_{i,d} (u), α_{i,d} (u) f^2);\\
...
H_{d+d'-1}^{∨}$, so that the anti-symmetry condition on the
